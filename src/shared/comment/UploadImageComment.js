...
async function commentImageOnComment(imageUrl, text, replyToCommentId, replyToPostId, replyToProfile, replyToUsername, imageHeight, imageWidth) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        await uploadTask.then(async(snapshot) => {
            ...
            await getDownloadURL(snapshot.ref).then(async (url) => {
                ...
                await saveCommentToComment(url, text, replyToCommentId, replyToProfile, replyToUsername, imageHeight, imageWidth)
                .then(async (id) => {

                    const result = { url: url, id: id };

                    // increment comments count on comment
                    const commentRef = doc(db, 'allPosts', replyToPostId, "comments", replyToCommentId);
...
// Comment image on a post
const saveCommentToComment = async (url, text, replyToCommentId, replyToPostId, replyToProfile, replyToUsername, imageHeight, imageWidth,) => {
...
export { commentImageOnPost, commentImageOnComment };